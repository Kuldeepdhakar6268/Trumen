...
ul li::before {
    ...
    background-image: url('{{asset(Storage::url('uploads/logo/listlogo2.jpg'))}}'); /* Image URL */
 ...
 <div class="Quotation" style="margin-top: 0px;" >
     <div class="Quotation-address">
       ...
       <div class="display">
        <table class="Quotation-table">
            ...
            <tbody>
                ...
                <p style="margin: 1rem 0 1rem 0;height: 50px;">{{$quotation->notes}}</p>
                <div>
       ...
       <div class="Quotation-footer" style="margin-top: 20.2rem;margin-bottom: 0.2rem;">
          <p style="line-height: 2.0;">TTPL/{{$year}}{{$yearNext->format('y')}}/{{$quotation->quotation_id}} {{\Carbon\Carbon::parse($quotation->quotation_date)->format('d.m.Y')}}</p>
        ...
        <table class="Quotation-table" style="border-collapse: collapse;">
           ...
           <?php
            ...
            //  dd($item);
             ...
             <tr>
                    <td></td>
                    <td><div style="text-decoration: underline; line-height: 3rem; font-weight: 550; text-align:center;"><p>{{$item->name}}</p><p>HSN Code: {{$item->hsn_code}}</p><p>Application: {{$item->application_text == ''?$item->application:$item->application_text}}</p></div></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                    <td><b style="padding: 0px 0px 0px 10px;">{{$key+1}}.0</b></td>
                    <td><b style="padding: 0px 0px 0px 10px;">Model: {{$item->new_model != ''?$item->new_model:$item->q_model}}</b></td>
                 ...
                 @if($item->Enclosure == null && $item->gland != '')
                 <tr>
                    <td></td>
                    <td><b style="padding: 0px 0px 0px 10px;">{{$item->Enclosure == ''?$item->gland:''}}</b></td>
                    <td></td>
...
// dd($integrals);
         ?>
                ...
                <tr>
                    <td></td>
                    <td class="speci_{{$keys}}"><b style="padding: 10px;">{{ $cs->prefix}}</b>@if($c->name == 'Enclosure') {{$integrals}} @endif   {{$c->name}}: 
