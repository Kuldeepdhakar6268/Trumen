        ...
        <tr>
            ...
            <td rowspan="2" colspan="2" class="commTD"><div class="display"><span>
              @if($quotation->jobcard_no != 0)
                ...
                SO-@php echo sprintf('%02d',$quotation->jobcard_no); @endphp
                @else
                SO-@php echo sprintf('%02d', $quotation->order_no == 0?$quotation->order_no+1:$quotation->order_no); @endphp
                @endif  
                
                </span><img src="{{$quotation->barcode}}" alt="Red dot" style="width: 130px;height: 56px;"/></td>
            <td class="commTD">Old SO_Ref</td>
            <td class="commTD">{{$quotation->old_ref_no != null?$quotation->old_ref_no:'SO-'sprintf('%02d',$jobNo)}}</td>
            <tr>
            <td class="commTD">Quote Ref</td>
            <td class="commTD">TTPL/{{$year}}{{$yearNext->format('y')}}/@php echo sprintf('%02d', $quotation->order_no == 0?$quotation->order_no+1:$quotation->order_no); @endphp</td>
            </tr>
        </tr>
        <tr>
