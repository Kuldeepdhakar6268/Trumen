...
class ProductServiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(\Auth::user()->can('manage product & service'))
        {
            ...
            if(!empty($request->category))
            {
                ...
                $productServices = ProductService::where('created_by', '=', \Auth::user()->creatorId())->where('category_id', $request->category)->with(['category','unit','code','group','material'])->orderBy('id', 'desc')->get();
            }
            else
            {
    ...
    public function create()
    {
        if(\Auth::user()->can('create product & service'))
        {
            ...
            $model = ProductModel::get()->pluck('name', 'id');
            ...
            return view('productservice.create', compact('category', 'group', 'unit', 'tax', 'customFields','incomeChartAccounts','incomeSubAccounts','expenseChartAccounts' , 'expenseSubAccounts', 'product_services', 'model'));
        }
        else
        {
     ...
     if($request->sendforapp == '')
     {
        if(\Auth::user()->can('create product & service'))
        {
            ...
            $rules = [
                ...
                'base_price' => 'required',
                'base_price_usd' => 'required',
                'base_price_euro' => 'required',
                'hsn_code' =>'required',
            ...
            if($validator->fails())
            {
                ...
                return redirect()->route('productservice.create')->with('error', $messages->first());
            }
//   dd($request->all());
            ...
            // $productCodeMaterial->usd_unit_rate                = $request->material_total_usd_price[$key];
            // $productCodeMaterial->euro_unit_rate                = $request->material_total_euro_price[$key];
            ...
            $productService->name                = $array[0].':'.$request->name;
            $productService->model          = $request->model;
            $productService->product_model_id          = $request->product_model;
            ...
            $productService->base_price          = $request->base_price;
            $productService->is_integral          = $request->is_integral;
            $productService->base_price_usd          = $request->base_price_usd;
            $productService->base_price_euro          = $request->base_price_euro;
            $productService->usd_sale_price          = $request->usd_price;
            $productService->euro_sale_price          = $request->euro_price;
...
//                $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
                ...
                $invoiceProduct->price = $request->sale_price;
   ...
   public function invoiceNumber()
    ...
    public function edit($id)
    {
       ...
       $htmls = '';
        ...
        //   dd($cats);
            ...
            $htmls ='<div class="col-md-2 comm-div-first edit-input">
            <div class="form-group">
        ...
        <div class="col-md-2 comm-div-first edit-input">
            <div class="form-group">
        ...
        <div class="col-md-2 comm-div-first edit-input">
            <div class="form-group">
                <label for="usd_unit_rate" class="form-label">Unit Rate ($)</label><span class="text-danger">*</span>
                <input class="form-control " required="required" step="0.01" name="usd_unit_rate[]" type="number" value="' . $cat->usd_unit_rate . '" id="usd_unit_rate-'.$cat->id.'" >
            </div>
        </div>
         <div class="col-md-2 comm-div-first edit-input">
            <div class="form-group">
                <label for="euro_unit_rate" class="form-label">Unit Rate (€)</label><span class="text-danger">*</span>
                <input class="form-control " required="required" step="0.01" name="euro_unit_rate[]" type="number" value="' . $cat->euro_unit_rate . '" id="euro_unit_rate-'.$cat->id.'">
            </div>
        </div>
         <div class="col-md-1 comm-div-first edit-input">
            <div class="form-group">
         ...
         <div class="col-md-1 comm-div-first edit-input">
            <div class="form-group">
                <label for="material_total_price" class="form-label">Total ($)</label><span class="text-danger">*</span>
                <input class="form-control number-inputs" required="required" readonly="" name="material_total_usd_price[]" type="text" value="' . $cat->usd_unit_rate . '" id="material_total_price-'.$cat->id.'">
            </div>
        </div>
         <div class="col-md-1 comm-div-first edit-input">
            <div class="form-group">
                <label for="material_total_price" class="form-label">Total (€)</label><span class="text-danger">*</span>
                <input class="form-control number-inputss" required="required" readonly="" name="material_total_euro_price[]" type="text" value="' . $cat->euro_unit_rate . '" id="material_total_price-'.$cat->id.'">
            </div>
        </div>
        ';
        ...
        if(\Auth::user()->can('edit product & service'))
        {
            ...
            $expenseSubAccounts = $expenseSubAccounts->get()->toArray();
                ...
                return view('productservice.edit', compact('category', 'unit', 'group', 'tax', 'data', 'datas', 'cats', 'productService', 'customFields','incomeChartAccounts','expenseChartAccounts' , 'incomeSubAccounts' , 'expenseSubAccounts', 'model'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function update(Request $request, $id)
    {
        ...
        if(\Auth::user()->can('edit product & service'))
        {
            ...
            // dd($productCode);
            //   dd($request->all());
            if(count($request->material) == count($request->material_id))
            {
            ...
            SpecificationCodeMaterial::where("id", $request->material_id[$key])->update([
                ...
                'usd_unit_rate' => $request->material_total_usd_price[$key],
                'euro_unit_rate' => $request->material_total_euro_price[$key],
                'qty' =>1
            ...
            {
               return redirect()->back()->with('error', __('Please select right group that have same specifications.'));  
            }
            if($productCode)
            {
            ...
            $productService->name                = $request->name;
            $productService->model          = $request->model;
            $productService->product_model_id          = $request->product_model_id;
            $productService->hsn_code          = $request->hsn_code;
            $productService->description         = $request->description;
            $productService->sku                 = 'PRD'.'-'.rand(9999, 10000);
            $productService->sale_price          = $request->sale_price;
            ...
            $productService->base_price_usd          = $request->base_price_usd;
            $productService->base_price_euro          = $request->base_price_euro;
            $productService->usd_sale_price          = $request->usd_sale_price;
            $productService->is_integral          = $request->is_integral;
            $productService->euro_sale_price          = $request->euro_sale_price;
            $productService->purchase_price      = $request->purchase_price;
            $productService->tax_id              = '';
            $productService->unit_id             = 1;
            $productService->group_id             =  $request->group_id;
