...
class ToDoController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            $last_30days = Todo::where('created_by', $usr->id)->orderBy('created_at', 'desc')->get();
            // Deal Summary
            // $cnt_deal                = [];
            ...
            return view('deals.index', compact('last_30days'));
        }
        else
        {
    ...
    public function create()
    {
        ...
        if(\Auth::user()->type == 'sales' || \Auth::user()->type == 'company')
        {
           
            $settings = Utility::settings();

            return view('task.create', compact('settings'));
        }
        else
        {
    ...
    public function getReminder()
    {
        ...
        if(\Auth::user()->type == 'sales' || \Auth::user()->type == 'company')
        {
            ...
            $settings = Utility::settings();
            return view('task.createreminder', compact('settings'));
        }
        else
        {
            return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function getCalReminder()
    {
        
        if(\Auth::user()->type == 'sales' || \Auth::user()->type == 'company')
        {
            $data = Todo::where('created_by', Auth()->User()->id)->where('reminder', 1)->orderBy('created_at', 'desc')->get();
            $arrayJson = [];
            foreach ($data as $val) {
                ...
                $arrayJson[] = [
                    ...
                    "textColor" => '#51459d',
                    "allDay" => true,
                    'url' => route('reminder.index', $val->id),
 ...
 /**
     ...
     * @return \Illuminate\Http\Response
     */
    public function getReadReminder(Request $request)
    {
       
       $find = Todo::find($request->id);
       $find->is_read = $find->is_read == 0?1:0;
       $find->save();
       if($find){
          return response()->json(
                    [
                        'is_success' => true,
                        'error' => __('Reminder viewed'),
                    ], 200
                ); 
       }else{
           return response()->json(
