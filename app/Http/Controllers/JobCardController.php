...
//             $pipelines = Pipeline::where('created_by', '=', $usr->ownerId())->get()->pluck('name', 'id');
            $deals = JobCard::select('deals.*','users.name as client', 'users.id as client_id','stages.name as stage_name')
            ->join('deals', 'deals.id', '=', 'jobcards.deal_id')
            ->leftJoin('client_deals', 'client_deals.deal_id', '=', 'deals.id')
            ->leftJoin('users', 'client_deals.client_id', '=', 'users.id')
            ->leftJoin('stages', 'deals.stage_id', '=', 'stages.id')
            ->paginate(10);
            // $jobcards  = JobCard::with('deals')->get();
           
// dd($deals);
            return view('jobcards.list', compact('deals'));
...
//     }
    ...
    public function sendJobcardEmail($id)
    {
            $ids = Crypt::decrypt($id);
            $quote = Quotation::find($ids);
            $quote->is_send = $quote->is_send + 1;
            $quote->save();
            ...
            $yearNext = \Carbon\Carbon::parse($years)->addYears(1);
                ...
                $dealEmail =
                    [
                        'deal_name' => $customer->name,
                        'to' => $customer->email,
                        'subject' => 'Jobcard Acknowledgement',
                        'description' => 'Acknowledgement has been sent to the '.$customer->name.' your JobCard No. is: TTPL/'.$year.''.$yearNext->format('y').'/'.sprintf('%02d', $quote->order_no == 0?$quote->order_no+1:$quote->order_no),
                        'link' =>'<a href="'.route('jobcard.download', ['id' => $ids]).'" targate="__blank">Your Jobcard pdf link here, Please click on link for download pdf</a>'
                    ];
                ...
                [
                    ...
                    $dd = Mail::to($customer->email)->send(new SendJobcardEmail($dealEmail));
                ...
                ActivityLog::create(
                    [
                        'user_id' => \Auth::user()->id,
                        'deal_id' => $quote->id,
                        'log_type' => 'Create Jobcard Email',
                        'remark' => json_encode(['title' => 'Create new Jobcard Email']),
                    ]
                );

                return redirect()->back()->with('success', __('Email has been send successfully !') . ((isset($smtp_error)) ? '<br> <span class="text-danger">' . $smtp_error . '</span>' : ''))->with('status', 'emails');
            // }
            // else
            // {
