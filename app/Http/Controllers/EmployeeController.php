...
class EmployeeController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->can('manage employee'))
        {
            ...
            {
                $employees = Employee::where('user_id', '=', Auth::user()->id)->with(['designation','branch','department'])->orderBy('id', 'desc')->get();
            }
            else
            {
                $employees = Employee::where('created_by', \Auth::user()->creatorId())->with(['designation','branch','department'])->orderBy('id', 'desc')->get();
            }
            // dd($employees);
...
//            $employeesId  = \Auth::user()->employeeIdFormat($employee->employee_id);
            ...
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents','departmentData', 'roles'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//                        $path = $request->file('document')[$key]->storeAs('uploads/document/', $fileNameToStore);
            ...
            if(!empty($user)){
                ...
                $user->type =  $request['role'] == 16?'sales':(($request['role'] == 10)?'user':(($request['role'] == 12)?'customer':(($request['role'] == 8)?'company':(($request['role'] == 15)?'HR':'employee'))));
