...
class ProductSpecificationController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('create product & service'))
        {
            ...
            return view('productspecification.create', compact('group', 'model'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create product & service'))
        {
            ...
            $rules = [
                ...
                'usd_price.*' => 'required|between:0,99.99',
                'euro_price.*' => 'required|between:0,99.99',
            ...
            $productService->el_mc                = $request->check;
            $productService->is_length                = $request->length;
            ...
            $productService->parent       = $request->parent;
            ...
            if(count($request->prefix)>0) {
                ...
                foreach($request->prefix as $key => $v)
                {
                    ...
                    $productSub->usd_price                = $request->usd_price[$key];
                    $productSub->euro_price                = $request->euro_price[$key];
                    $productSub->belongs                = $request->belongs[$key];
                    $productSub->prefix                = $request->prefix[$key];
                    $productSub->length_price                = $request->length_price[$key];
                    $productSub->group_id = $request->group_id;
                    $productSub->product_model_id = $request->product_model_id;
                    $productSub->parent       = $request->parent;
                    $productSub->child_id       = $request->child_id;
    ...
    public function edit($id)
    {
        ...
        $group = Group::where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');
        $model = ProductModel::get()->pluck('name', 'id');
        $models = Specification::where(['priority' => 0, 'product_model_id' => $productService->product_model_id])->get();
        ...
        if(\Auth::user()->can('edit product & service'))
        {
            ...
            {
                return view('productspecification.edit', compact('productService', 'subproductService', 'group', 'model', 'models'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
...
//                            }
                ...
                $productService->product_model_id = $request->product_model_id;
                $productService->parent       = $request->parent;
                $productService->child_id       = $request->child_id;
                $productService->save();
                
                if(count($request->s_id)>0) {
                foreach($request->prefix as $key => $v)
                {
                   if($key<= count($request->s_id)-1)
                   {
                    ...
                    $productSubNew->name                = $request->sub_specification[$key];
                    $productSubNew->priority       =$id;
                    $productSubNew->price                = $request->price[$key];
                    $productSubNew->usd_price                = $request->usd_price[$key];
                    $productSubNew->euro_price                = $request->euro_price[$key];
                    $productSubNew->prefix                = $request->prefix[$key];
                    $productSubNew->group_id = $request->group_id;
                    $productSubNew->product_model_id = $request->product_model_id;
            ...
            // if(count($request->prefix_new)>0 && $request->prefix_new[0] != null)
            // {
                
            //      foreach($request->prefix_new as $key => $v)
            //     {
                   
            //         $productSubNew               = new Specification();
            //         $productSubNew->name                = $request->sub_specifications_new[$key];
            //         $productSubNew->priority       =$id;
            //         $productSubNew->price                = $request->price_new[$key];
            //         $productSubNew->usd_price                = $request->usd_price_new[$key];
            //         $productSubNew->euro_price                = $request->euro_price_new[$key];
            //         $productSubNew->prefix                = $request->prefix_new[$key];
            //         $productSubNew->group_id = $request->group_id;
            //         $productSub->product_model_id = $request->product_model_id;
            //         $productSubNew->created_by       = \Auth::user()->creatorId();
            //         $productSubNew->save();  
