...
class DashboardController extends Controller
{
    ...
    public function landingPage()
    {
        ...
        if ($adminSettings['display_landing_page'] == 'off' && \Schema::hasTable('landing_page_settings')) {
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                    ...
                    $data['quote'] = Quotation::get();
                        $data['monthlyOrderCount'] = Quotation::selectRaw('COUNT(*) as count, YEAR(quotation_date) year, MONTH(quotation_date) month')
                        ->groupBy('year', 'month')
                        ->get();
                        $data['totalManOrder'] = Quotation::where(
                            'created_at', '<', Carbon::now()->firstOfMonth()->toDateTimeString()
                        )->where(['is_jobcard' => 1, 'created_by' => \Auth::user()->creatorId()])->count();
                        $data['lastMonth_pendingOrder'] = Quotation::where(
                            'created_at', '<', Carbon::now()->firstOfMonth()->toDateTimeString()
                        )->where(['order_status' => 'Pending', 'is_jobcard' => 1, 'created_by' => \Auth::user()->creatorId()])->count();
                         $data['lastMonth_ongoingOrder'] = Quotation::where(
                            'created_at', '<', Carbon::now()->firstOfMonth()->toDateTimeString()
                        )->where(['order_status' => 'On-Going', 'is_jobcard' => 1, 'created_by' => \Auth::user()->creatorId()])->count();
                         $data['lastMonth_completeOrder'] = Quotation::where(
                            'created_at', '<', Carbon::now()->firstOfMonth()->toDateTimeString()
                        )->where(['order_status' => 'Complete', 'is_jobcard' => 1, 'created_by' => \Auth::user()->creatorId()])->count();
                    if(Auth()->User()->type == 'company')
                    {
                     $data['todo'] = Todo::orderBy('created_at','desc')->where('reminder', 0)->get();
                     $data['remind'] = Todo::orderBy('created_at','desc')->where('reminder', 1)->get();
                     $data['findremind'] = Todo::where(['created_by' => Auth::user()->id])->orderBy('created_at','desc')->whereDate('start', now()->format('Y-m-d'))->where(['reminder' => 1, 'is_read' => 0])->where('time', '<=', now()->format('H:i'))->first();
                    }else{
                     $data['todo'] = Todo::where(['created_by' => Auth::user()->id])->orderBy('created_at','desc')->where('reminder', 0)->get();
                     $data['remind'] = Todo::where(['created_by' => Auth::user()->id])->orderBy('created_at','desc')->where('reminder', 0)->get();
                     $data['findremind'] = Todo::where(['created_by' => Auth::user()->id])->orderBy('created_at','desc')->whereDate('start', now()->format('Y-m-d'))->where(['reminder' => 1, 'is_read' => 0])->where('time', '<=', now()->format('H:i'))->first();
                    }
                    // material stock 
                    
                   $data['materials'] = MaterialStock::all();
                     $deadstock = MaterialStock::selectRaw('round(((current_qty - dead_material_qty)/current_qty)*100, 1) net_qty')->where('created_by', '=', \Auth::user()->creatorId())->get()->toArray();
                      $materialName = MaterialStock::where('created_by', '=', \Auth::user()->creatorId())->pluck('material_name')->toArray();
                    ...
                    $startDate = \Carbon\Carbon::now()->subDays(1)->startOfDay()->format('Y-m-d');
                    $createdUsers = \App\Models\Customer::whereDate('created_at', '<=', $startDate)->where('is_customer', 1)->count(); // 2
                    $createdTodayUsers = \App\Models\Customer::where('created_at', '=', $date)->where('is_customer', 1)->count(); // 2
                    $todayCount = $createdTodayUsers == 0?$createdTodayUsers+1:$createdTodayUsers;
                    $data['customerPer'] = (($todayCount/$createdUsers)  * 100);
                    
                    $createdOrders = \App\Models\Quotation::whereDate('created_at', '<=', $startDate)->where('is_order', 1)->count(); // 2
                    $createdTodayOrders = \App\Models\Quotation::where('created_at', '=', $date)->where('is_order', 1)->count(); // 2
                    $todayOrderCount = $createdTodayOrders == 0?$createdTodayOrders+1:$createdTodayOrders;
                    $data['orderPer'] = (($todayOrderCount/$createdOrders)  * 100);
                    $data['createdOrders'] =$createdOrders;
                    $quote = \App\Models\Quotation::selectRaw('sum(quotation_products.price) as amount')->leftjoin('quotation_products', 'quotations.id', '=', 'quotation_products.quotation_id')->where('quotations.is_jobcard', '=', 1)->whereDate('quotations.quotation_date', '=', \Carbon\Carbon::parse($date))->get();
                    $quoteT = \App\Models\Quotation::selectRaw('sum(quotation_products.price) as amount')->leftjoin('quotation_products', 'quotations.id', '=', 'quotation_products.quotation_id')->where('quotations.is_jobcard', '=', 1)->whereDate('quotations.updated_at', '=', \Carbon\Carbon::parse($date))->get();
                        
                    $amt = $quoteT[0]->amount == ''?$quoteT[0]->amount+1:$quoteT[0]->amount;
                    $amta = $quote[0]->amount == ''?$quote[0]->amount+1:$amt - $quote[0]->amount;
                        
                    $data['per'] = ($amt /$amta) * 100; 
                    $dd[] = $data['totalManOrder'];
                    // $dd = array_push($dd,$data['totalManOrder']);
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
 ...
 public function sales_dashboard_index()
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.sales&crm-dashboard', $data);
                } else {
    ...
    public function purchase_management_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.purchase-management-dashboard', $data);
                } else {
    ...
    public function production_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.production-dashboard', $data);
                } else {
    ...
    public function manufacturing_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.manufacturing-dashboard', $data);
                } else {
    ...
    public function disptch_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.disptch-dashboard', $data);
                } else {
    ...
    public function inventory_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.inventory-dashboard', $data);
                } else {
    ...
    public function hrms_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.hrms-dashboard', $data);
                } else {
    ...
    public function reports_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.reports-dashboard', $data);
                } else {
    ...
    public function finance_dashboard_index()
    {
        if(Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                   ...
                   $data['materials'] = MaterialStock::all();
                    ...
                    $data['incTotalMan'] = $dd;
                    //  dd($data);
                    return view('dashboard.finance-dashboard', $data);
                } else {
