        ...
        if(\Auth::user()->can('create vender'))
        {
            ...
            if($validator->fails())
            {
                ...
                if($total_vendor < $plan->max_venders || $plan->max_venders == -1)
                {
                    ...
                    $vender->company_name             = $request->name;
                    $vender->name             = $request->contact_person;
                    $vender->contact          = $request->contact;
                    $vender->alternate          = $request->alternate;
                    $vender->sector          = $request->sector;
                    $vender->industry          = $request->industry;
                    $vender->email            = $request->email;
                    $vender->tax_number      =$request->tax_number;
                    $vender->plot_number      =$request->plot_number;
                    $vender->land_mark      =$request->land_mark;
                    $vender->street_name      =$request->street_name;
                    $vender->created_by       = \Auth::user()->creatorId();
                    $vender->billing_name     = $request->contact_person;
                    $vender->billing_country  = $request->country;
                    $vender->billing_state    = $request->state;
                    $vender->billing_city     = $request->city;
                    $vender->billing_phone    = $request->contact;
                    $vender->billing_zip      = $request->pincode;
                    $vender->billing_address  = $request->area_name;
                    $vender->shipping_name    = $request->contact_person;
                    $vender->shipping_country = $request->country;
                    $vender->shipping_state   = $request->state;
                    $vender->shipping_city    = $request->city;
                    $vender->shipping_phone   = $request->shipping_phone;
                    $vender->shipping_zip     = $request->pincode;
