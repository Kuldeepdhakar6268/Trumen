...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function purchaseNumberFormat($number)
    {
        ...
        return $settings["purchase_prefix"] . sprintf("%02d", $number);
    }
    public static function quotationNumberFormat($number)
    {
        ...
        return $settings["quotation_prefix"] .'/'.$year.$yearNext->format('y').'/'. sprintf("%02d", $number);
    }
    //  public static function requestNumberFormat($number)
    // {
    //     return $settings["order_prefix"] . sprintf("%05d", $number);
    // }
    public static function quotationRvNumberFormat($number)
    {
        $settings = Utility::settings();
        $year = \Carbon\Carbon::now()->format('y');
        $years= \Carbon\Carbon::now()->format('Y-m-d');
        $yearNext = \Carbon\Carbon::parse($years)->addYears(1);
        return $settings["quotation_prefix"] .'/'.$year.$yearNext->format('y').'/RV-'. sprintf("%02d", $number);
    }
     ...
     public static function jobNumberFormat($number)
    {
        ...
        return $settings["job_prefix"] . sprintf("%02d", $number);
    }
     public static function soNumberFormat($number)
    {
        $settings = Utility::settings();

        return $settings["so_prefix"] . sprintf("%02d", $number);
    }
    ...
    public function countConvertedCustomers()
    {
        return Customer::where(['created_by' => $this->creatorId(), 'is_customer' => 1])->count();
    }
